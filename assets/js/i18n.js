...
const TRANSLATIONS = {
  en: {
    ...
    "home.bio2":        "My current work spans two areas. In <strong>RL for drug discovery</strong>, I am building a discrete masked diffusion model that combines RL-based property optimization and natural language conditioning. In <strong>loss of plasticity</strong>, I am studying why deep RL agents degrade in learning capacity over time and whether evolutionary-inspired mechanisms can restore it. More broadly, I am drawn to questions about causal structure in RL, particularly whether latent variable models that recover causal environment dynamics could improve agent generalization, and how RL can steer generative models toward open problems in scientific discovery.",
    "home.bio3":        "Before Mila I spent 4+ years at <a href='https://www.db.com'>Deutsche Bank</a>'s Chief Innovation Office in Pune, working on financial AI for regulatory requirements, LLM agents, fine-tuning LLMs for internal deployment, and safety evaluation of LLM systems. I hold a Bachelor's in Information Technology from <a href=\"https://www.nitk.ac.in/\">NIT Karnataka</a>.",
    "home.news":        "News",
    ...
    "cv.db.detail":     "Financial AI for regulatory requirements, LLM agents for internal use cases, fine-tuning LLMs for internal deployment, and safety evaluation of LLM systems.",
    ...
    "questions.q0a":               "<strong>Warm-up-derived adaptive thresholds.</strong> UCB-style methods set $T(s) = Q(s) + \\lambda \\cdot \\sigma_Q(s)$ globally. The open question is whether a threshold learned during a warm-up phase can serve as a structured prior: exploit when $Q(s, a) \\geq T(s)$, explore when $Q(s, a) < T(s)$. Does this warm-up prior give a meaningful advantage over a UCB baseline with no such phase?",
    "questions.q0b":               "<strong>Nearest-neighbour generalisation for unseen states.</strong> For states never visited during warm-up, the threshold can be approximated via $\\text{Explore}(s) = \\mathbb{1}[Q(s,a) < T(\\text{NN}(s))]$. How does the choice of distance metric affect generalisation quality, and does it outperform learned uncertainty estimates such as ensemble disagreement or Bayesian Q-network epistemic uncertainty?",
    ...
    "questions.q3":                "<strong>Recursive reasoning in latent chemical space.</strong> Can a model iteratively refine a molecular latent vector $z_{t+1} = f(z_t, c)$ conditioned on text $c$ before decoding — analogous to chain-of-thought but operating over a continuous chemical manifold? Gated attention or a SteerViT-style mechanism could mediate this.",
  ...
  fr: {
    ...
    "home.bio3":        "Avant Mila, j'ai passé plus de 4 ans au <a href='https://www.db.com'>Deutsche Bank</a> Chief Innovation Office, travaillant sur l'IA financière pour les exigences réglementaires, les agents LLM, le fine-tuning de LLM pour déploiement interne, et l'évaluation de la sécurité des systèmes LLM. J'ai obtenu un B.Tech. en informatique du <a href=\"https://www.nitk.ac.in/\">NIT Karnataka</a>.",
    ...
    "cv.db.detail":     "IA financière pour les exigences réglementaires, agents LLM pour des cas d'usage internes, fine-tuning de LLM pour déploiement interne, et évaluation de la sécurité des systèmes LLM.",
    ...
    "questions.q0a":               "<strong>Seuils adaptatifs dérivés de l'échauffement.</strong> Les méthodes UCB fixent $T(s) = Q(s) + \\lambda \\cdot \\sigma_Q(s)$ globalement. La question ouverte est de savoir si un seuil appris lors d'une phase d'échauffement peut servir de prior structuré pour les interactions ultérieures.",
    "questions.q0b":               "<strong>Généralisation par plus proche voisin pour les états non visités.</strong> Pour les états jamais visités pendant l'échauffement, le seuil peut être approximé via $\\text{Explore}(s) = \\mathbb{1}[Q(s,a) < T(\\text{NN}(s))]$. Comment le choix de la métrique de distance affecte-t-il la qualité de généralisation ?",
    ...
    "questions.q1":                "<strong>Représentations auto-supervisées comme pont.</strong> L'apprentissage contrastif peut rapprocher les molécules structurellement similaires et éloigner les dissemblables — sans étiquettes texte. L'objectif est $\\mathcal{L} = -\\log\\dfrac{\\exp(\\text{sim}(z_i, z_j)/\\tau)}{\\sum_k \\exp(\\text{sim}(z_i, z_k)/\\tau)}$. Ces représentations peuvent-elles servir pour la génération conditionnée par le texte ?",
    ...
    "questions.q3":                "<strong>Raisonnement récursif dans l'espace latent chimique.</strong> Un modèle peut-il affiner itérativement un vecteur latent $z_{t+1} = f(z_t, c)$ conditionné par un texte $c$ avant le décodage, à l'image du chain-of-thought mais sur un manifold chimique continu ?",
    ...
    "questions.q8":                "<strong>Mises à jour de gradient sélectives par réponse.</strong> Un agent peut-il appliquer un masque de mise à jour signé $M_t \\in \\{-1, 0, +1\\}$ aux segments de réponse — descente de gradient sur les parties jugées mauvaises, ascente sur les bonnes ? Comment $M_t$ doit-il être décidé ?",
